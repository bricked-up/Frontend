  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (me.data) {
        ...
        const orgsRaw = await Promise.all(orgIds.map((oid) => getOrg(oid).then((r) => r.data)));
    ...
    <>
      <Box sx={{ p: 3 }}>
        <Tab
          ...
          iconPosition="start"
          label="Organizations"
          sx={{ color: theme.palette.mode === "dark" ? colors.greenAccent[400] : colors.blueAccent[600] }}
        />
            ...
            <>
              ...
              <List dense>
                {members.map((m, i) => (
                  ...
                  >
                    <ListItemText primary={m.name} primaryTypographyProps={{ color: "text.primary" }} />
                  </ListItem>
                ))}
              </List>
            </>
