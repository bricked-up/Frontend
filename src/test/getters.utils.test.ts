...
describe("Getter Utility Functions (Integration Tests)", () => {
    ...
    it("should get existing user data successfully (status 200)", async () => {
      ...
      expect(result.data).toEqual(
        expect.objectContaining({
          id: existingUserId,
          email: "jane.smith@example.com", // Adjust to your test data
          name: "Jane Smith", // Adjust to your test data
          avatar: "avatar2.png", // Adjust to your test data
          verified: true,
  ...
  describe("getAllUsers", () => {
    it("should get all user IDs successfully (status 200)", async () => {
      ...
      if (result.data && result.data.length > 0) {
        ...
        console.warn(
          "getAllUsers returned empty or null data. This may be valid."
        );
      }
    });
  });
  ...
  describe("getProject", () => {
    ...
    it("should get existing project data successfully (status 200)", async () => {
      ...
      if (
        (result.data as Project).issues &&
        (result.data as Project).issues!.length > 0
      ) {
  ...
  describe("getProjectMember", () => {
    ...
    it("should get existing project member data and map fields (status 200)", async () => {
      ...
      if (memberData.roles) {
        // roles on ProjectMember is number[]
      ...
      if (memberData.issues) {
        // issues on ProjectMember is number[]
